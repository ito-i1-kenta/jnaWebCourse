...
public class AssigneeRegisterControllerTest {
    ...
    @Autowired
    AssigneeRegisterController target;
    ...
    public void normal_test() throws Exception {
        ...
        doNothing().when(assigneeService).save(assigneeName);
        when(registerService.getAssigneeList()).thenReturn(assigneeResponse.getAssigneeEntityList());

        mockMvc.perform(MockMvcRequestBuilders.post("/assigneeRegister")
                .param("insert", assigneeName))

                // レスポンスのステータスコードが200であることを検証する
                .andExpect(status().isOk())
                .andExpect(view().name("assigneeViewer"));

        verify(assigneeService).save(assigneeName);
    }
}
